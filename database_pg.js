...
Database.createProject = function(owner, title, description, sector, owner_name, callback) {
	pg.connect(PG_DATABASE_URL, function(err, client, done) {
		if (err) {
			done();
			callback(err);
		...
		client.query(`SELECT * FROM projects WHERE title = '${title}'`).on('end', function(result) {
			if (result.rowCount > 0) {
				...
				client.query(`INSERT INTO projects (id, owner, title, description, sector, owner_name) VALUES (uuid_generate_v4(), '${owner}', '${title}', '${description}', '${sector}', '${owner_name}')`);
				client.query(`SELECT * FROM projects WHERE title = '${title}'`).on('end', function(result) {
...
Database.editProject = function(id, title, description, sector, callback) {
	pg.connect(PG_DATABASE_URL, function(err, client, done) {
		if (err) {
			done();
			callback(err);
		...
		client.query(`SELECT * FROM projects WHERE (title = '${title}' AND id != '${id}')`).on('end', function(result) {
			if (result.rowCount > 0) {
				...
				client.query(`UPDATE projects SET title = '${title}', description = '${description}', sector = '${sector}' WHERE id = '${id}'`);
				client.query(`SELECT * FROM projects WHERE title = '${title}'`).on('end', function(result) {
					done();
					callback(null, result.rows[0]);
				});
