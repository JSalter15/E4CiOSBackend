...
Database.getPostsForCategory = function(category, postType, callback) {
	pg.connect(PG_DATABASE_URL, function(err, client, done) {
		...
		var finalResults = [];
		client.query(`SELECT term_id FROM wp_terms WHERE name = '${category}'`).on('end', function(result) {

			if (result.rowCount == 0) callback("category does not exist");
			else {
				let termid = result.rows[0].term_id;
				let query = client.query(`SELECT object_id FROM wp_term_relationships WHERE term_taxonomy_id = ${termid}`);
